...
class FlpDualScaledLSSVM(object):

    def __init__(self, lambd=1, lr=0.1, max_iter=50, kernel="linear", degree=None) -> None:
        super().__init__()
        self.lr = lr
        self.degree = degree
        self.lambd = lambd
    ...
    def fit(self, X, y):
        ...
        # Kappa computation
        # self.kappa = 1 / (self.delta * (self.data.shape[0] ** 2) * (self.data.shape[1] ** 2))
        self.kappa = 1 / ((self.data.shape[0]) * (self.data.shape[1]))
        ...
        for i in range(self.max_iter):
            ...
            r_k = np.dot(q_k.T, q_k) / np.dot(q_k.T, np.dot(opt_matrix, q_k))
